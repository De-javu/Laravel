...
<!------------------------------------------------------------- Diccionario de terminos para esta practica ------------------------------------------------------------------------------>
  ...
  <!-- condicional para mostrar la imagen o un mensaje de error si no se encuentra -->
@if (isset($image->user->image_path)) //? Se encarga de validar si la imagen del usuario existe
<img src="{{ route('image.file', ['filename' => $image->image_path]) }}" //? Se encarga de mostrar la imagen del usuario
alt="Imagen" class="object-cover w-full h-auto mx-auto"> //? Se encarga de mostrar la imagen del usuario


<!-- Botonones para editar y eliminar la imagen -->
@if ($image->user->id == Auth::user()->id) //? Se encarga de validar si el usario logeado es el dueño de la imagen
<div class="p-1 "> //? Se crea una clase para pa poner padding
<span class="text-green-500">{{ 'NOMBRE DE LA IMAGEN =   ' . $image->image_path}}</span> <br> //? Se encraga de mostrar el nombre de la imagen     
<x-nav-link href="{{ route('image.edit', ['id' => $image->id]) }}"> //? Se encarga de redirigir a la vista de editar imagen
<x-secondary-button>{{'Editar'}}</x-secondary-button></x-nav-link>  //? Se crea el boton editar                                  


<!-- >Integracion del boton  modal -->
<x-danger-button type="button"  onclick="openModal('exampleModal')">Eliminar</x-danger-button > //? Se crea el boton de eliminar la imagen con la funcion de abrir el modal                            
</div> 


<!-- Button trigger modal -->
<div id="exampleModal" class="fixed inset-0 flex items-center justify-center z-50 hidden"> //? Se crea el modal para eliminar la imagen
<div class="modal-overlay absolute inset-0 bg-gray-900 opacity-80"></div> //? Se crea el fondo del modal
<div class="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto"> //? Se crea el contenedor del modal
<div class="modal-content py-4 text-left px-6"> //? Se crea el contenido del modal


<!-- Modal header -->
<div class="flex justify-between items-center pb-3 font-mono "> //? Se crea el header del modal
<button type="button" class="text-black close-modal" onclick="closeModal('exampleModal')"> //? Se crea el boton de cerrar el modal
<span aria-hidden="true">&times;</span> //? Se crea el icono de cerrar el modal
</button> 
<br>
<h1 class="text-2xl font-bold text-gray-800" id="exampleModalLabel">Estas seguro de eliminar esta imagen!</h1>  //? Se crea el titulo del modal


</div>
<!-- Modal body mensaje -->
<div class="my-5"> //? Se crea el cuerpo del modal
<div class="flex items center text-gray-500"> //? Se crea el mensaje del modal
<p>Eliminar una imagen es una acción permanente que no puede deshacerse. Antes de proceder, es importante asegurarse de que la imagen ya no es necesaria</p> //? Se crea el mensaje del modal
</div>
</div>
<!-- Modal footer accion de eliminar configuracion vista-->
<div class="flex justify-end pt-2"> //? Se crea el footer del modal
<x-secondary-button  class="m-4" onclick="closeModal('exampleModal')">Close</x-secondary-button> //? Se crea el boton de cerrar el modal
<x-nav-link href="{{route('image.destroy', ['id' => $image->id])}}"> //? Se crea la ruta para eliminar el modal
<x-danger-button class="m-2">{{ __('Eliminarrrrr') }}</x-danger-button> //? Se crea el boton de eliminar la imagen
</x-nav-link>
</div>
</div>
</div>
...
{{-- Configuracion del Modal con js para actuar con el boton de eliminar --}}
<script> //? Se crea el script para abrir y cerrar el modal
function openModal(modalId) { //? Se crea la funcion para abrir el modal
document.getElementById(modalId).classList.remove('hidden'); //? Se encarga de quitar la clase hidden para mostrar el modal
}

function closeModal(modalId) { //? Se crea la funcion para cerrar el modal
document.getElementById(modalId).classList.add('hidden'); //? Se encarga de agregar la clase hidden para cerrar el modal
} 
</script> //? Se cierra el script para abrir y cerrar el modal
